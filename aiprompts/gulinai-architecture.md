# Gulin AI Architecture Documentation

## Overview

Gulin AI is a chat-based AI assistant feature integrated into Gulin Terminal. It provides a conversational interface for interacting with various AI providers (OpenAI, Anthropic, Perplexity, Google, and Gulin's cloud proxy) through a unified streaming architecture. The feature is implemented as a block view within Gulin Terminal's modular system.

## Architecture Components

### Frontend Architecture (`frontend/app/view/gulinai/`)

#### Core Components

**1. GulinAiModel Class**
...
**3. React Components**
- **GulinAi**: Main container component
...
aiOpts: Atom<GulinAIOptsType>      // Final AI options for requests
```
...
├── Create GulinAIStreamRequest
├── Call RpcApi.StreamGulinAiCommand()
...
### Backend Architecture (`pkg/gulinai/`)
...
**3. GulinAICloudBackend** (`cloudbackend.go`)
- **Provider**: Gulin's cloud proxy service
- **Transport**: WebSocket connection to Gulin cloud
- **Features**: 
...
**Command**: `streamgulinai`
**Type**: Response Stream (one request, multiple responses)

**Request Type** (`GulinAIStreamRequest`):
```go
...
**Response Type** (`GulinAIPacketType`):
```go
...
**Frontend**:
- **Method**: `fetchGulinFile(blockId, "aidata")`
- **Format**: JSON array of `GulinAIPromptMessageType`
- **Sliding Window**: Last 30 messages (`slidingWindowSize = 30`)

**Backend**:
- **Service**: `BlockService.SaveGulinAiData(blockId, history)`
...
**Stored Messages** (`GulinAIPromptMessageType`):
```go
...
- **Cloud**: Gulin's proxy (no token/baseURL)
