...
import { focusedGulinWindow, getAllGulinWindows } from "./emain-window";
...
export class Updater {
    ...
    constructor(settings: SettingsType) {
        ...
        autoUpdater.on("update-downloaded", (event) => {
            ...
            const updateNotification = new Notification({
                title: "Gulin Terminal",
                body: "A new version of Gulin Terminal is ready to install.",
            });
    ...
    private set status(value: UpdaterStatus) {
        this._status = value;
        getAllGulinWindows().forEach((window) => {
        ...
        if (
            ...
            if (userInput && !result.downloadPromise) {
                ...
                if (focusedGulinWindow) {
                    dialog.showMessageBox(focusedGulinWindow, dialogOpts);
                }
            }
    ...
    async promptToInstallUpdate() {
        ...
        if (allWindows.length > 0) {
            await dialog.showMessageBox(focusedGulinWindow ?? allWindows[0], dialogOpts).then(({ response }) => {
                if (response === 0) {
