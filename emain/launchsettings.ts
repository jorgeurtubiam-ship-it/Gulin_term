...
import { getGulinConfigDir } from "./emain-platform";

/**
 * Get settings directly from the Gulin Home directory on launch.
...
export function getLaunchSettings(): SettingsType {
    const settingsPath = path.join(getGulinConfigDir(), "settings.json");
    try {
