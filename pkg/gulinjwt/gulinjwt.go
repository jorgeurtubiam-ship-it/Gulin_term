...
func ValidateAndExtract(tokenStr string) (*GulinJwtClaims, error) {
	globalLock.Lock()
	pubKey := publicKey
	globalLock.Unlock()
	...
	token, err := jwt.ParseWithClaims(tokenStr, &GulinJwtClaims{}, func(token *jwt.Token) (interface{}, error) {
	...
	claims, ok := token.Claims.(*GulinJwtClaims)
	if !ok || !token.Valid {
...
func Sign(claims *GulinJwtClaims) (string, error) {
	globalLock.Lock()
	privKey := privateKey
	globalLock.Unlock()
