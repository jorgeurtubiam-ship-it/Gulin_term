...
var gulinObjUpdateKey = struct{}{}

type contextUpdatesType struct {
	UpdatesStack []map[ORef]GulinObjUpdate
}
...
func ContextWithUpdates(ctx context.Context) context.Context {
	updatesVal := ctx.Value(gulinObjUpdateKey)
	if updatesVal != nil {
		return ctx
	}
	return context.WithValue(ctx, gulinObjUpdateKey, &contextUpdatesType{
		UpdatesStack: []map[ORef]GulinObjUpdate{make(map[ORef]GulinObjUpdate)},
	})
}

func ContextGetUpdates(ctx context.Context) map[ORef]GulinObjUpdate {
	updatesVal := ctx.Value(gulinObjUpdateKey)
	if updatesVal == nil {
		return nil
	}
	...
	rtn := make(map[ORef]GulinObjUpdate)
...
func ContextGetUpdate(ctx context.Context, oref ORef) *GulinObjUpdate {
	updatesVal := ctx.Value(gulinObjUpdateKey)
	if updatesVal == nil {
		return nil
	}
...
func ContextAddUpdate(ctx context.Context, update GulinObjUpdate) {
	updatesVal := ctx.Value(gulinObjUpdateKey)
	if updatesVal == nil {
		return
	}
...
func ContextUpdatesBeginTx(ctx context.Context) context.Context {
	...
	updates := updatesVal.(*contextUpdatesType)
	updates.UpdatesStack = append(updates.UpdatesStack, make(map[ORef]GulinObjUpdate))
	return ctx
}
