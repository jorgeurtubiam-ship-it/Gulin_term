...
type UpdatesRtnType = []GulinObjUpdate
...
type GulinObjUpdate struct {
	UpdateType string  `json:"updatetype"`
	OType      string  `json:"otype"`
	OID        string  `json:"oid"`
	Obj        GulinObj `json:"obj,omitempty"`
}

func (update GulinObjUpdate) MarshalJSON() ([]byte, error) {
...
func MakeUpdate(obj GulinObj) GulinObjUpdate {
	return GulinObjUpdate{
...
func MakeUpdates(objs []GulinObj) []GulinObjUpdate {
	rtn := make([]GulinObjUpdate, 0, len(objs))
...
func (update *GulinObjUpdate) UnmarshalJSON(data []byte) error {
	...
	if _, found := objMap["updatetype"]; !found {
		return fmt.Errorf("missing updatetype (in GulinObjUpdate)")
	}
	update.UpdateType, ok1 = objMap["updatetype"].(string)
	if !ok1 {
		return fmt.Errorf("in GulinObjUpdate bad updatetype type %T", objMap["updatetype"])
	}
	if _, found := objMap["otype"]; !found {
		return fmt.Errorf("missing otype (in GulinObjUpdate)")
	}
	update.OType, ok2 = objMap["otype"].(string)
	if !ok2 {
		return fmt.Errorf("in GulinObjUpdate bad otype type %T", objMap["otype"])
	}
	if _, found := objMap["oid"]; !found {
		return fmt.Errorf("missing oid (in GulinObjUpdate)")
	}
	update.OID, ok3 = objMap["oid"].(string)
	if !ok3 {
		return fmt.Errorf("in GulinObjUpdate bad oid type %T", objMap["oid"])
	}
	if _, found := objMap["obj"]; found {
		objMap, ok := objMap["obj"].(map[string]any)
		if !ok {
			return fmt.Errorf("in GulinObjUpdate bad obj type %T", objMap["obj"])
		}
		gulinObj, err := FromJsonMap(objMap)
		if err != nil {
			return fmt.Errorf("in GulinObjUpdate error decoding obj: %w", err)
		}
		update.Obj = gulinObj
	}
	return nil
...
type Job struct {
	...
	JobAuthToken    string            `json:"jobauthtoken"` // job manger -> gulin
	AttachedBlockId string            `json:"attachedblockid,omitempty"`
	GulinVersion     string            `json:"gulinversion,omitempty"`
...
func AllGulinObjTypes() []reflect.Type {
	return []reflect.Type{
