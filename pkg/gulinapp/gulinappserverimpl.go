...
type GulinAppServerImpl struct {
	Client  *Client
	BlockId string
}

func (*GulinAppServerImpl) WshServerImpl() {}

func (impl *GulinAppServerImpl) VDomRenderCommand(ctx context.Context, feUpdate vdom.VDomFrontendUpdate) chan wshrpc.RespOrErrorUnion[*vdom.VDomBackendUpdate] {
	...
	for _, event := range feUpdate.Events {
		if event.GlobalEventType != "" {
			...
			impl.Client.Root.Event(event.GulinId, event.EventType, event)
		}
	}
	// update refs
...
func (impl *GulinAppServerImpl) VDomUrlRequestCommand(ctx context.Context, data wshrpc.VDomUrlRequestData) chan wshrpc.RespOrErrorUnion[wshrpc.VDomUrlRequestResponse] {
	...
	go func() {
		...
		if httpReq.URL.Path == "/gulin/global.css" && impl.Client.GlobalStylesOption != nil {
