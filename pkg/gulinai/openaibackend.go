...
func setApiType(opts *wshrpc.GulinAIOptsType, clientConfig *openaiapi.ClientConfig) error {
...
func convertPrompt(prompt []wshrpc.GulinAIPromptMessageType) []openaiapi.ChatCompletionMessage {
...
func (OpenAIBackend) StreamCompletion(ctx context.Context, request wshrpc.GulinAIStreamRequest) chan wshrpc.RespOrErrorUnion[wshrpc.GulinAIPacketType] {
	...
	go func() {
		...
		for {
			...
			if streamResp.Model != "" && !sentHeader {
				pk := MakeGulinAIPacket()
				pk.Model = streamResp.Model
				pk.Created = streamResp.Created
				rtn <- wshrpc.RespOrErrorUnion[wshrpc.GulinAIPacketType]{Response: *pk}
				sentHeader = true
			}
