...
func (AnthropicBackend) StreamCompletion(ctx context.Context, request wshrpc.GulinAIStreamRequest) chan wshrpc.RespOrErrorUnion[wshrpc.GulinAIPacketType] {
	...
	go func() {
		...
		for {
			...
			case "message_delta":
				...
				if event.Usage != nil {
					pk := MakeGulinAIPacket()
					pk.Usage = &wshrpc.GulinAIUsageType{
