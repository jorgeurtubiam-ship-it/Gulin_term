...
func (GoogleBackend) StreamCompletion(ctx context.Context, request wshrpc.GulinAIStreamRequest) chan wshrpc.RespOrErrorUnion[wshrpc.GulinAIPacketType] {
	...
	go func() {
		...
		for {
			...
			rtn <- wshrpc.RespOrErrorUnion[wshrpc.GulinAIPacketType]{Response: wshrpc.GulinAIPacketType{Text: convertCandidatesToText(resp.Candidates)}}
		}
	}()
	return rtn
}

func extractHistory(history []wshrpc.GulinAIPromptMessageType) []*genai.Content {
...
func extractPrompt(prompt []wshrpc.GulinAIPromptMessageType) genai.Part {
