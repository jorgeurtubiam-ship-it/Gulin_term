...
# Add Gulin binary directory to PATH
...
wsh token "$GULIN_SWAPTOKEN" fish 2>/dev/null | source
set -e GULIN_SWAPTOKEN

# Load Gulin completions
wsh completion fish | source

set -g _GULIN_SI_FIRSTPROMPT 1

# shell integration
function _gulin_si_blocked
...
function _gulin_si_osc7
    _gulin_si_blocked; and return
    # Use fish-native URL encoding
    set -l encoded_pwd (string escape --style=url -- "$PWD")
    printf '\033]7;file://localhost%s\007' $encoded_pwd
end

function _gulin_si_prompt --on-event fish_prompt
    set -l _gulin_si_status $status
    _gulin_si_blocked; and return
    if test $_GULIN_SI_FIRSTPROMPT -eq 1
    ...
    set -g _GULIN_SI_FIRSTPROMPT 0
end

function _gulin_si_preexec --on-event fish_preexec
    _gulin_si_blocked; and return
...
function _gulin_si_chpwd --on-variable PWD
    _gulin_si_osc7
end