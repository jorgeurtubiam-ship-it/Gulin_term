...
const (
	GulinConfigHomeEnvVar           = "GULIN_CONFIG_HOME"
	GulinDataHomeEnvVar             = "GULIN_DATA_HOME"
	GulinAppPathVarName             = "GULIN_APP_PATH"
	GulinAppResourcesPathVarName    = "GULIN_RESOURCES_PATH"
	GulinAppElectronExecPathVarName = "GULIN_ELECTRONEXECPATH"
	GulinDevVarName                 = "GULIN_DEV"
	GulinDevViteVarName             = "GULIN_DEV_VITE"
	GulinWshForceUpdateVarName      = "GULIN_WSHFORCEUPDATE"
	GulinNoConfirmQuitVarName       = "GULIN_NOCONFIRMQUIT"

	GulinJwtTokenVarName  = "GULIN_JWT"
	GulinSwapTokenVarName = "GULIN_SWAPTOKEN"
)
...
var ConfigHome_VarCache string          // caches GULIN_CONFIG_HOME
var DataHome_VarCache string            // caches GULIN_DATA_HOME
var AppPath_VarCache string             // caches GULIN_APP_PATH
var AppResourcesPath_VarCache string    // caches GULIN_RESOURCES_PATH
var AppElectronExecPath_VarCache string // caches GULIN_ELECTRONEXECPATH
var Dev_VarCache string                 // caches GULIN_DEV

const GulinLockFile = "gulin.lock"
const DomainSocketBaseName = "gulin.sock"
const RemoteDomainSocketBaseName = "gulin-remote.sock"
const GulinDBDir = "db"
const ConfigDir = "config"
const RemoteGulinHomeDirName = ".gulin"
const RemoteWshBinDirName = "bin"
const RemoteFullWshBinPath = "~/.gulin/bin/wsh"
const RemoteFullDomainSocketPath = "~/.gulin/gulin-remote.sock"
...
var gulinCachesDirOnce = &sync.Once{}
var gulinCachesDir string
...
func CacheAndRemoveEnvVars() error {
	...
	os.Unsetenv(GulinDataHomeEnvVar)
	AppPath_VarCache = os.Getenv(GulinAppPathVarName)
	os.Unsetenv(GulinAppPathVarName)
	AppResourcesPath_VarCache = os.Getenv(GulinAppResourcesPathVarName)
	os.Unsetenv(GulinAppResourcesPathVarName)
	AppElectronExecPath_VarCache = os.Getenv(GulinAppElectronExecPathVarName)
	os.Unsetenv(GulinAppElectronExecPathVarName)
	Dev_VarCache = os.Getenv(GulinDevVarName)
	os.Unsetenv(GulinDevVarName)
	os.Unsetenv(GulinDevViteVarName)
	os.Unsetenv(GulinNoConfirmQuitVarName)
	return nil
}
...
func GetGulinAppElectronExecPath() string {
...
func GetDomainSocketName() string {
	return filepath.Join(GetGulinDataDir(), DomainSocketBaseName)
}
...
func GetPersistentRemoteSockName(clientId string) string {
	return fmt.Sprintf("~/.gulin/client/%s/gulin.sock", clientId)
}

func EnsureGulinDataDir() error {
	return CacheEnsureDir(GetGulinDataDir(), "gulinhome", 0700, "gulin home directory")
}
...
func EnsureGulinPresetsDir() error {
	return CacheEnsureDir(filepath.Join(GetGulinConfigDir(), "presets"), "gulinpresets", 0700, "gulin presets directory")
}

func resolveGulinCachesDir() string {
	var cacheDir string
	appBundle := "gulin"
	if IsDevMode() {
		appBundle = "gulin-dev"
...
func GetGulinCachesDir() string {
	gulinCachesDirOnce.Do(func() {
		gulinCachesDir = resolveGulinCachesDir()
	})
	return gulinCachesDir
}

func EnsureGulinCachesDir() error {
	return CacheEnsureDir(GetGulinCachesDir(), "gulincaches", 0700, "gulin caches directory")
}
...
func determineLang() string {
	...
	if runtime.GOOS == "darwin" {
		...
		// this is specifically to get the gulinsrv LANG so gulinshell
...
func GetRemoteJobLogDir() string {
	homeDir := GetHomeDir()
	jobDir := filepath.Join(homeDir, ".gulin", "jobs")
	return jobDir
}
