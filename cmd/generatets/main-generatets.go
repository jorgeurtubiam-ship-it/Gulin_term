...
import (
	...
	"github.com/gulindev/gulin/pkg/tsgen"
...
func generateTypesFile(tsTypesMap map[reflect.Type]string) error {
	...
	tsgen.GenerateGulinObjTypes(tsTypesMap)
	tsgen.GenerateGulinEventTypes(tsTypesMap)
...
func generateGulinEventFile(tsTypesMap map[reflect.Type]string) error {
	fileName := "frontend/types/gulinevent.d.ts"
	fmt.Fprintf(os.Stderr, "generating gulinevent file to %s\n", fileName)
	var buf bytes.Buffer
	fmt.Fprintf(&buf, "// Copyright 2026, Command Line Inc.\n")
	fmt.Fprintf(&buf, "// SPDX-License-Identifier: Apache-2.0\n\n")
	fmt.Fprintf(&buf, "// generated by cmd/generate/main-generatets.go\n\n")
	fmt.Fprintf(&buf, "declare global {\n\n")
	fmt.Fprint(&buf, utilfn.IndentString("    ", tsgen.GenerateGulinEventTypes(tsTypesMap)))
	fmt.Fprintf(&buf, "}\n\n")
	fmt.Fprintf(&buf, "export {}\n")
	written, err := utilfn.WriteFileIfDifferent(fileName, buf.Bytes())
	if !written {
...
func main() {
	...
	err = generateGulinEventFile(tsTypesMap)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating gulin event file: %v\n", err)
		os.Exit(1)
	}
