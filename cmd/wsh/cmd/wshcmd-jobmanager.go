...
import (
	...
	"github.com/gulindev/gulin/pkg/jobmanager"
)

var jobManagerCmd = &cobra.Command{
	Use:    "jobmanager",
	Hidden: true,
	Short:  "job manager for gulin terminal",
	Args:   cobra.NoArgs,
	RunE:   jobManagerRun,
}
...
func jobManagerRun(cmd *cobra.Command, args []string) error {
	...
	publicKeyB64 := os.Getenv("GULIN_PUBLICKEY")
	if publicKeyB64 == "" {
		return fmt.Errorf("GULIN_PUBLICKEY environment variable is not set")
	}

	publicKeyBytes, err := base64.StdEncoding.DecodeString(publicKeyB64)
	if err != nil {
		return fmt.Errorf("failed to decode GULIN_PUBLICKEY: %v", err)
	}
