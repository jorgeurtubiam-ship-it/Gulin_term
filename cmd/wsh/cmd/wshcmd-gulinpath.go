...
var gulinpathCmd = &cobra.Command{
	Use:     "gulinpath {config|data|log}",
	Short:   "Get paths to various gulin files and directories",
	RunE:    gulinpathRun,
	PreRunE: preRunSetupRpcClient,
}

func init() {
	gulinpathCmd.Flags().BoolP("open", "o", false, "Open the path in a new block")
	gulinpathCmd.Flags().BoolP("open-external", "O", false, "Open the path in the default external application")
	gulinpathCmd.Flags().BoolP("tail", "t", false, "Tail the last 100 lines of the log")
	rootCmd.AddCommand(gulinpathCmd)
}

func gulinpathRun(cmd *cobra.Command, args []string) (rtnErr error) {
	defer func() {
		sendActivity("gulinpath", rtnErr == nil)
	}()

	if len(args) == 0 {
		OutputHelpMessage(cmd)
		return fmt.Errorf("no arguments. wsh gulinpath requires a type argument (config, data, or log)")
	}
	if len(args) > 1 {
		OutputHelpMessage(cmd)
		return fmt.Errorf("too many arguments. wsh gulinpath requires exactly one argument")
	}
