...
import { atoms, createBlock, fetchGulinFile, getApi, globalStore, WOS, getSettingsKeyAtom } from "@/store/global";
...
function promptToMsg(prompt: GulinAIPromptMessageType): ChatMessageType {
    return {
...
class AiWshClient extends WshClient {
    ...
    constructor(blockId: string, model: GulinAiModel) {
...
export class GulinAiModel implements ViewModel {
    viewType: string;
    blockId: string;
    ...
    constructor(blockId: string, nodeModel: BlockNodeModel, tabModel: TabModel) {
        ...
        this.viewText = atom((get) => {
                ...
                default:
                    if (isCloud) {
                        viewTextChildren.push({
                            elemtype: "iconbutton",
                            icon: "cloud",
                            title: "Using Gulin's AI Proxy (gpt-5-mini)",
                            noAction: true,
                        });
                    } else {
            ...
            viewTextChildren.push({
                elemtype: "menubutton",
                text: presetName,
                title: "Select AI Configuration", // This should also be translated but it's passed as a title string
                items: dropdownItems,
            });
    ...
    async fetchAiData(): Promise<Array<GulinAIPromptMessageType>> {
        const { data } = await fetchGulinFile(this.blockId, "aidata");
        if (!data) {
            return [];
        }
        const history: Array<GulinAIPromptMessageType> = JSON.parse(new TextDecoder().decode(data));
    ...
    sendMessage(text: string, user: string = "user") {
        ...
        const newPrompt: GulinAIPromptMessageType = {
            role: "user",
            content: text,
        };
        const handleAiStreamingResponse = async () => {
            ...
            const history = await this.fetchAiData();
            const beMsg: GulinAIStreamRequest = {
                clientid: clientId,
                opts: opts,
            ...
            try {
                ...
                if (fullMsg == "") {
                    ...
                    await BlockService.SaveGulinAiData(this.blockId, [...history, newPrompt]);
                } else {
                    const responsePrompt: GulinAIPromptMessageType = {
                        role: "assistant",
                        content: fullMsg,
                    };
                    ...
                    await BlockService.SaveGulinAiData(this.blockId, [...history, newPrompt, responsePrompt]);
                }
            } catch (error) {
                ...
                globalStore.set(this.updateLastMessageAtom, "", false);
                const errorPrompt: GulinAIPromptMessageType = {
                    role: "error",
                    content: errMsg,
                };
                updatedHist.push(errorPrompt);
                await BlockService.SaveGulinAiData(this.blockId, updatedHist);
            }
...
const GulinAi = ({ model }: { model: GulinAiModel; blockId: string }) => {
    const { sendMessage } = model.useGulinAi();
    const { t } = useTranslation();
    const gulinaiRef = useRef<HTMLDivElement>(null);
    ...
    const handleTextAreaKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        const gulinEvent = adaptFromReactOrNativeKeyEvent(e);
        if (checkKeyPressed(gulinEvent, "Enter")) {
            e.preventDefault();
            handleEnterKeyPressed();
        } else if (checkKeyPressed(gulinEvent, "ArrowUp")) {
            handleArrowUpPressed(e);
        } else if (checkKeyPressed(gulinEvent, "ArrowDown")) {
            handleArrowDownPressed(e);
        }
    };

    let buttonClass = "gulinai-submit-button";
    let buttonIcon = makeIconClass("arrow-up", false);
    let buttonTitle = "run";
    if (locked) {
        buttonClass = "gulinai-submit-button stop";
    ...
    return (
        <div ref={gulinaiRef} className="gulinai">
            {isUsingProxy && (
                <div className="flex items-start gap-3 px-4 py-2 bg-orange-500/25 border-b border-orange-500/50 text-sm">
                    ...
                    <span className="text-primary/90">
                        Gulin AI Proxy is deprecated and will be removed. Please use the new{" "}
                        <button
            ...
            <div className="gulinai-chat">
                <ChatWindow ref={osRef} chatWindowRef={chatWindowRef} msgWidths={msgWidths} model={model} />
            </div>
            <div className="gulinai-controls">
                <div className="gulinai-input-wrapper">
                    <ChatInput
                        ref={inputRef}
                        value={value}
                        model={model}
