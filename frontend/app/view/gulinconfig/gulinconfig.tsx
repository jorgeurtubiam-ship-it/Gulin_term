...
import type { ConfigFile, GulinConfigViewModel } from "@/app/view/gulinconfig/gulinconfig-model";
...
const GulinConfigView = memo(({ blockId, model }: ViewComponentProps<GulinConfigViewModel>) => {
    const selectedFile = useAtomValue(model.selectedFileAtom);
    ...
    const handleEditorMount = useCallback(
        (editor: MonacoTypes.editor.IStandaloneCodeEditor) => {
            ...
            const keyDownDisposer = editor.onKeyDown((e: MonacoTypes.IKeyboardEvent) => {
                const gulinEvent = adaptFromReactOrNativeKeyEvent(e.browserEvent);
                const handled = tryReinjectKey(gulinEvent);
                if (handled) {
                    e.stopPropagation();
                    e.preventDefault();
                }
    ...
    useEffect(() => {
        const handleKeyDown = keydownWrapper((e: GulinKeyboardEvent) => {
                            ...
                            <div className="flex items-baseline gap-2 min-w-0">
                                ...
                                {selectedFile.docsUrl && (
                                    <Tooltip content="View documentation">
                                        <a
                                            href={`${selectedFile.docsUrl}?ref=gulinconfig`}
                                            target="_blank"
                            ...
                            {isLoading ? (
                              ...
                              (!selectedFile.hasJsonView || activeTab === "visual") ? (
                                ...
                                <CodeEditor
                                    blockId={blockId}
                                    text={fileContent}
                                    fileName={`GULINCONFIGPATH/${selectedFile.path}`}
...
GulinConfigView.displayName = "GulinConfigView";

export { GulinConfigView };
