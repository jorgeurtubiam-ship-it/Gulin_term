...
class DBConnectionsViewModel implements ViewModel {
    ...
    async loadData() {
        ...
        try {
            ...
            const resp = await fetch(`${endpoint}/wave/db-list`, { headers });
            if (!resp.ok) return;
    ...
    async selectConnection(connName: string | null) {
        ...
        try {
            const endpoint = getWebServerEndpoint();
            const headers = { "X-AuthKey": getApi().getAuthKey() };
            const resp = await fetch(`${endpoint}/wave/db-schema?connection=${encodeURIComponent(connName)}`, { headers });
            if (!resp.ok) return;
    ...
    async exploreTable(connName: string, tableName: string) {
        try {
            ...
            const endpoint = getWebServerEndpoint();
            const headers = { "X-AuthKey": getApi().getAuthKey() };
            const url = `${endpoint}/wave/db-query?connection=${encodeURIComponent(connName)}&sql=${encodeURIComponent(sql)}&tabid=${encodeURIComponent(this.tabModel.tabId)}`;
