...
Gulin's configuration files are located at `~/.config/gulin/`.

The main configuration file is `settings.json` (`~/.config/gulin/settings.json`).
...
The easiest way to edit your config files is to use the wsh editconfig command which will open your Gulin config file in our built-in preview editor.
...
| ------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| app:globalhotkey                     | string   | A systemwide keybinding to open your most recent gulin window. This is a set of key names separated by `:`. For more info, see [Customizable Systemwide Global Hotkey](#customizable-systemwide-global-hotkey)                                                 |
| app:dismissarchitecturewarning       | bool     | Disable warnings on app start when you are using a non-native architecture for Gulin. For more info, see [Why does Gulin warn me about ARM64 translation when it launches?](./faq#why-does-gulin-warn-me-about-arm64-translation-when-it-launches).              |
...
| app:confirmquit <VersionBadge version="v0.14" />                     | bool     | Set to false to disable the quit confirmation dialog when closing Gulin Terminal (defaults to true, requires app restart)                                                                                                                                      |
| app:language <VersionBadge version="v0.15" />                        | string   | The application language code (`en` for English, `es` for Spanish). Defaults to `en`.                                                                                                                                                                         |
...
| ai:name                              | string   | string to display in the Gulin AI block header                                                                                                                                                                                                                 |
...
| ai:proxyurl                          | string   | HTTP proxy URL for AI API requests (does not apply to Gulin Cloud AI)                                                                                                          |
...
```json
{
    ...
    "web:defaulturl": "https://github.com/gulindev/gulin",
    ...
    "term:durable": false,
    "gulinai:showcloudmodes": true,
    "gulinai:defaultmode": "gulinai@balanced"
}
```

:::warning

If you installed Gulin pre-v0.9.0 your configuration file will be located at
`~/.gulin/config/settings.json`. This includes all of the other configuration
...
To avoid putting secrets directly in config files, Gulin supports environment variable resolution using `$ENV:VARIABLE_NAME` or `$ENV:VARIABLE_NAME:fallback` syntax.  This works for any string value in any config file (settings.json, presets.json, ai.json, etc.).
...
```json
{
  ...
  "bookmark@gulin-github": {
    "url": "https://github.com/gulindev/gulin",
    "title": "Gulin Github",
  ...
  "bookmark@gulin-pulls": {
    "url": "https://github.com/gulindev/gulin/pulls",
    "title": "Gulin Pull Requests",
    "iconurl": "https://github.githubassets.com/favicons/favicon-dark.png"
  }
}
```
...
User-defined terminal themes are located in `~/.config/gulin/termthemes.json`.
...
Here is an example of defining a full terminal theme. All of the built-in themes are defined here: https://github.com/gulindev/gulin/blob/main/pkg/wconfig/defaultconfig/termthemes.json (if you'd like to add a popular terminal theme, please submit a PR!)
...
Gulin allows settings a custom global hotkey to open your most recent window from anywhere in your computer. This has the name `"app:globalhotkey"` in the `settings.json` file and takes the form of a series of key names separated by the `:` character.

### Examples

As a practical example, suppose you want a value of `F5` as your global hotkey. Then you can simply set the value of `"app:globalhotkey"` to `"F5"` and reboot Gulin to make that your global hotkey.
