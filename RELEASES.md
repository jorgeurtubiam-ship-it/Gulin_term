...
1. Go to the [Actions tab](https://github.com/gulindev/gulin/actions) and select "Bump Version" from the left sidebar.
...
3. After "Bump Version" a "Build Helper" run will kick off automatically for the new version. When this completes, it will generate a [draft GitHub Release](https://github.com/gulindev/gulin/releases) with all the built artifacts.
...
Our release builds are managed by the [Build Helper workflow](https://github.com/gulindev/gulin/actions/workflows/build-helper.yml).

Under the hood, this will call the `package` task in [`Taskfile.yml`](./Taskfile.yml), which will build the `gulinsrv` and `wsh` binaries, then the frontend and Electron codebases using Vite, then it will call `electron-builder` to generate the distributable app packages. The configuration for `electron-builder` is defined in [`electron-builder.config.cjs`](./electron-builder.config.cjs).

This will also sign and notarize the macOS app packages and sign the Windows packages.

Once a build is complete, the artifacts will be placed in `s3://gulin-github-artifacts/staging-w2/<version>`. A new draft release will be created on GitHub and the artifacts will be uploaded there too.

### Testing new releases

The [Build Helper workflow](https://github.com/gulindev/gulin/actions/workflows/build-helper.yml). creates a draft release on GitHub once it completes. You can find this on the [Releases page](https://github.com/gulindev/gulin/releases) of the repo. You can use this to download the build artifacts for testing.
